<template>
    <Layout bodyCls="f-column" :border="false">
        ...
        <Dialog ref="selectApproverDlg" closed
                :title="'选择审批人'"
                :draggable="true"
                :resizable="true"
                :dialogStyle="{width:'40%',height:'40%'}"
                :modal="true">
            <selectOrganizationUser ref="selectOrganizationUserCom" @selectUser="selectApprover"></selectOrganizationUser>
        </Dialog>
        ...
        <Dialog ref="selectMainBodyDlg" closed
                :title="'选择主体及分类'"
                :draggable="true"
                :resizable="true"
                :dialogStyle="{width:'600px',height:'600px'}"
                :modal="true">
            <!--            <ComboBox @selectionChange="selectMainBody($event)" :data="mainBodyList" textField="name" valueField="id" style="width: 100%"></ComboBox>-->
            <Layout bodyCls="f-column" :border="false">
                <LayoutPanel region="west" style="width:200px;" bodyCls="f-column" :border="true">
                    <Tree :data="mainBodyList" @selectionChange="selectMainBody($event)"></Tree>
                </LayoutPanel>
                <LayoutPanel region="east" style="width:200px;" bodyCls="f-column" :border="true">
                    <Tree :data="brandList" @selectionChange="selectBrand($event)"></Tree>
                </LayoutPanel>
                <LayoutPanel region="center" style="height:100%;" bodyCls="f-column" :border="true">
                    <Tree :data="typeList" @selectionChange="selectType($event)"></Tree>
                </LayoutPanel>
            </Layout>
            <template slot="footer">
                <div class="text-center p-5">
                    <LinkButton @click="submit">提交</LinkButton>
                    <LinkButton @click="$refs.selectMainBodyDlg.close()">关闭</LinkButton>
                </div>
            </template>
        </Dialog>
    </Layout>
...
export default {
    ...
    created: function () {
        this.userObj = this.$root.userObj;
    ...
    methods: {
        ...
        selectUser(obj) {
            ...
            let vm = this;
            this.confirm("转给" + obj.username + ",确认吗?", function () {
                vm.getData("commoditynew/assignStaff", {rows: JSON.stringify(arr), inUserid: obj.id}, function (data) {
                    vm.assignStaffId=data;
                    vm.$refs.selectUserDlg.close();
                    vm.$refs.selectApproverDlg.open();
                })
            })
        },
        selectApprover(obj){
            let vm = this;
            this.confirm("发送"+obj.username+"转出审核,确认吗?", function () {
                vm.getData("commoditynew/sendApprover", {id:vm.assignStaffId,approverId:obj.id}, function (data) {
                    vm.allSelected=false;
                    vm.$refs.selectApproverDlg.close();
        ...
        submit() {
            let err = '';
            if (!this.submitObj.mainBodyId) {
                err += '请选择主体<br>';
            }
            if (!this.submitObj.brandId) {
                err += '请选择品牌<br>';
            }
            if (!this.submitObj.typeId) {
                err += '请选择类别<br>';
            }
            if (err) {
                this.alert(err);
                ...
                let vm = this;
                this.confirm("提交新SKU申请,确认吗?", function () {
                    vm.getData("commoditynew/push", {
                        id: vm.obj.id,
                        mainBodyId: vm.submitObj.mainBodyId,
                        typeId: vm.submitObj.typeId,
                        brandId: vm.submitObj.brandId
                    }, function (data) {
                        vm.$refs.selectMainBodyDlg.close();
                        vm.alert(data);
                    })
                })
            }
